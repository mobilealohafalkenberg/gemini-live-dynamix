...
const HTTP_ENDPOINT = process.env.REACT_APP_ER_BRIDGE_ENDPOINT || 'http://localhost:8082';
...
export interface UseBridgeWebSocketReturn {
  isConnected: boolean;
...
export function useBridgeWebSocket(handlers: MessageHandlers = {}): UseBridgeWebSocketReturn {
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    try {
      ...
      // Update local state and call handlers
      switch (type) {
